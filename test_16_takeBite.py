...
#                    (0,0)]
        ...
        boxV = body.fixtures[0].shape.vertices
        ...
        r = .25
        N = 20
        circV,circS = self.makeALD(POC,r,N) #circle segments not needed
        b = Polygon(circV)
...
#        newPolyS = self.XY2Verts_Segs(newPoly.exterior.coords)
        #newBody = 
#        x1,y1 = self.plotPolyPoints(circV,circS)
#        #plt.plot(x1,y1)
#        x2,y2 = self.plotPolyPoints(boxV,boxS)
#        plt.plot(x2,y2)
#        #plt.show()
#        inVerts,outVerts,SCVP = self.findInsidePoints(boxV,circV)
#        #print SCVP
#        inX,inY = self.verts2XY(inVerts)
#        outX,outY = self.verts2XY(outVerts)
