#FirstTest_AnimSpritesExampleIntegration

# First attempt at animating sprites in Pygame. Successfully modified several UN Squadron 
#sprite sheets and manually determined offsets that are set in code for animation frames. 
#Example allows cycling through animated sprites in a window with "ENTER" key.

# Also contains first attempt at object oriented approach to physics based entities with
#the intention of using them as the root classes for interactive game elements. This section
#is not complete and is being left as is while some off the shelf physics engines are evaluated.
#The goal is to create these classes ambiguously so that they can be used with any physics engine 
#so that existing ones can be tweaked to work with the real time FEA/SPH features intended for 
#this game.
...
class Particle():
    ...
    def __init__(self,properties,initialConditions,integrater = 'RK4'):
        self.mass = properties[0]
        self.radius = properties[1]
        
        self.position = initialConditions[0]
        self.velocity = initialConditions[1]
        self.acceleration = initialConditions[2]
        
        self.integrater = integrater
        
    def update(self):
        self.sumForces()
        self.calcNewState()
        
    def calcNewState(self):
        self.acceleration = self.getAcceleration()
        if self.intMethod == 'RK4':
            x=1
        elif self.intMethod == 'Euler':
            self.velocity = (self.acceleration(0)*dt,self.acceleration(1)*dt)
            
    def sumForces(self):
        fx,fy = 0,0
        for efx,efy in self.externalForces:
            fx = fx+efx
            fy = fy+efy
