...
def makeMyMesh(verts):
    """
    Returns constrained triangulation of a simple concave or convex polygon without holes.
    Does NOT currently use segs, creates them from verts assuming contiguous verts
...
#            ...)
    """
    nodes = [tmVertex(node) for node in verts]
    N = len(verts)
    segs = [(1+i,2+i) for i in range(N-1)]
    segs.append((N,1))
    segs = [tmSegmentId(seg) for seg in segs]
    md  = TriangleMesh()
    rtn = md.Mesh( nodes, segs, [])
    #md.PrintData()
    return md
