...
for i in range(len(parts)):
    tri = Delaunay(points[vertList[i]])
    tris = [0]*tri.vertices.shape[0]
    for j in range(tri.vertices.shape[0]):
        tris[j] = points[tri.vertices[j]]
        plt.plot(tris[j][:,0],tris[j][:,1])
    parts[i] = tris
    
p1[0].axes.set_aspect('equal')
plt.show()    
...
##    tris[i] = points[tri.vertices[i]]
##    print i
##    p1=plt.plot(tris[i][:,0],tris[i][:,1])
##p1[0].axes.set_aspect('equal')
##plt.show()


###tail1 = [cV[0],cV[1],cV[2]]
####tail2 = [,cV[11]]#,cV[0]]
###bottom = [cV[10],cV[12],cV[15],cV[7],cV[14],cV[13],cV[9]]
###top = [cV[11],cV[2],cV[4],cV[15]]
