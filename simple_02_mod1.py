...
def my_draw_polygon(polygon, body, fixture):
    ...
    #imX = baseImage.get_width()
    #print imX
    #bodyX = 40
    (x,y)= body.worldCenter*PPM
    #Ix,Iy = baseImage.get_size()
    print x,y
    #print bodyX
    #scale = 1#bodyX/imX
    #print scale
    ...
    rect = image.get_rect()
    #image = pygame.transform.rotozoom(baseImage, rot, scale)
    #image = baseImage
    #x = vertices[0][0]
    #y = vertices[0][1]
    #print(x)
    #fixX = 540
    #fixY = 419
    #screen.blit(image, (fixX,fixY))#vertices[0])
    #screen.blit(image, (x-(100*scale),y-(50*scale)))
    sprX, sprY = (x-(rect.width/2),-(rect.height)-y+SCREEN_HEIGHT)
    print ("sprite:" +str(sprX) +" " + str(sprY))
    print ("body  :" + str(x) + " " + str(y))
    screen.blit(image, (sprX,sprY))
    pygame.draw.polygon(screen, colors[body.type], vertices)
#    screen.blit(image, (fixX-100,fixY))
#    screen.blit(image, (fixX-100,fixY))
