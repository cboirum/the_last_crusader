...
class Pyramid (Framework):
    ...
    def __init__(self):
        ...
        position = (40,50)
        self.addMeshPyCrusader(position)
        #self.printStuff(self.ground.fixtures[20])
#        testFixture = self.ground.fixtures[200]
#        bb = testFixture.body
#        self.printStuff(bb)
        #print bb.position
        #self.ground.joints
        #print len(self.ground.fixtures)
        self.settings.drawContactPoints = True
        
        
        
    def addMeshPyCrusader(self, position):
        crusaderBase = Assets.meshpyCrusader(PPM,self.flightModel)
    ...
    def addPlayer(self):
        ...
        self.crusader1.userData = { 'ID': 'PlayerAircraft',
                                   'limitKE': 91703.315759}
        #self.crusader1 = self.world.
...
#        print relativeVelocityMag*relativeVelocityMag
        ...
        if specialContact:
            for C in [A,B]:
                #if 'limitKE' in C.userData.keys:
                if type(C.userData)==type({}):
                    if KE > C.userData['limitKE']:
...
#        pass  
    ...
    def PostSolve(self, contact, impulse):
        ...
        """
        #print self.points
        norm = impulse.normalImpulses
        tang = impulse.tangentImpulses
        normMag = math.sqrt(norm[0]*norm[0]+norm[1]*norm[1])
        tangMag = math.sqrt(tang[0]*tang[0]+tang[1]*tang[1])
        impulseMag = math.sqrt(normMag*normMag + tangMag*tangMag)
        #print impulseMag
        if impulseMag > 50:
            print impulseMag
